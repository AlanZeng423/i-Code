...
from .modules_video import SpatioTemporalAttention  # 导入卷积操作、注意力机制、视频机制
...
class TimestepBlock(nn.Module):
    ...
    @abstractmethod
    def forward(self, x, emb):  # 是一个抽象方法 所以此处不提供事项方法
        """
...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb=None, context=None, x_0=None):
        # 处理数据
        is_video = (x.ndim == 5)  # 检查输入数据是否为视频数据（视频数据有5个维度）
        if is_video:
            num_frames = x.shape[2]
            if emb is not None:
                # 在批次后面添加一个维度b，放入帧数num_frames，然后重新排列为b*num_frames
        ...
        # 用于定义将输入数据 x 通过神经网络的不同层进行传递和处理的方法
        for layer in self:
            if isinstance(layer, TimestepBlock):
                # layer默认为调用该类的forward方法
                x = layer(x, emb)
...
class UpsampleDeterministic(nn.Module):
    def __init__(self, upscale=2):  # 初始化方法。它接受一个参数 upscale，默认值为 2，表示上采样的放大倍数。
        super(UpsampleDeterministic, self).__init__()  # 继承并初始化了其父类 nn.Module 的基础功能。
        self.upscale = upscale  # TODO: why？

    def forward(self, x):
        return x[:, :, :, None, :, None] \
            .expand(-1, -1, -1, self.upscale, -1, self.upscale) \
            .reshape(x.size(0), x.size(1), x.size(2) * self.upscale, x.size(3) * self.upscale)
        # 先插入了新维度(34之间 和 4之后)，张量为[batch, channels, height, 1, width, 1]
        # expand用于扩展维度，-1的部分不改变，但self.upscale部分扩展为self.upscale的大小
        # reshape用于调整张量的形状
        # 通过复制像素来放大图像的高度和宽度，但不会增加新的信息或细节到图像中。
...
class Upsample(nn.Module):
    ...
    def __init__(self, channels, use_conv, dims=2, out_channels=None, padding=1):
        ...
        self.out_channels = out_channels or channels  # 输出的通道数，如果未指定，则默认为输入通道数。
        self.use_conv = use_conv  # 布尔值，确定是否应用卷积。
        self.dims = dims  # 信号的维度（1D、2D或3D）。
        if use_conv:
            # 如果确定使用卷积，创建一个卷积层。conv_nd 是一个函数，用于创建根据 dims 指定的维度的卷积层。
            self.conv = conv_nd(dims, self.channels, self.out_channels, 3, padding=padding)
        self.upsample = UpsampleDeterministic(2)  # 上采样操作

    def forward(self, x):
        assert x.shape[1] == self.channels  # 断言确保输入数据 x 的通道数与类初始化时指定的通道数相匹配。
        x = self.upsample(x)  # 首先对输入数据进行上采样
        if self.use_conv:
            x = self.conv(x)  # 上采样后对数据应用卷积层
        return x
...
class TransposedUpsample(nn.Module):
    ...
    def __init__(self, channels, out_channels=None, ks=5):
        ...
        #  创建一个 nn.ConvTranspose2d 实例用于上采样
...
class Downsample(nn.Module):
    ...
    def __init__(self, channels, use_conv, dims=2, out_channels=None, padding=1):
        super().__init__()
        self.channels = channels  # 赋值操作同Upsample
        self.out_channels = out_channels or channels
        self.use_conv = use_conv
        self.dims = dims
        stride = 2 if dims != 3 else (1, 2, 2)
        # "stride" 参数通常用于定义卷积层或池化层的步长，即在处理数据时滑动窗口移动的距离
        # stride 被设置为 2（或在3D情况下为 (1, 2, 2)深度1单位，高度和宽度2单位)，这意味着在每个非批次维度上数据大小减半，从而实现下采样。
        # 在1D和2D情况下，数据在所有维度上都缩小，而在3D情况下，仅在最后两个维度上进行缩小。
        if use_conv:
            self.op = conv_nd(
                dims, self.channels, self.out_channels, 3, stride=stride, padding=padding
            )  # 比Upsample中的卷积层多了stride部分
        else:
            assert self.channels == self.out_channels
            # 如果不使用卷积，则使用平均池化进行下采样。池化核大小和步长均为 stride。
...
class ConnectorOut(nn.Module):
    ...
    :param emb_channels: the number of timestep embedding channels. (此处没用上)
    ...
    def __init__(
        ...
        self.out_channels = out_channels or channels
        self.use_conv = use_conv  # 如果为 True 且指定了 out_channels，使用空间卷积而不是1x1卷积来改变残差连接中的通道。
        self.use_checkpoint = use_checkpoint  # 是否使用梯度检查点。

        self.in_layers = nn.Sequential(
            normalization(channels),  # 这个层用于对输入数据进行规范化，有助于稳定训练过程和加快收敛速度
            nn.SiLU(),  # 激活函数，一种非线性激活函数，有助于增加模型的表达能力。
        ...
        self.use_temporal_attention = use_temporal_attention  # 是否使用时间注意力。
        if use_temporal_attention:  # 如果使用时间注意力，创建一个时间注意力层。
        ...
        # 根据out_channels 和 use_conv 定义skip_connection——残差连接
        if self.out_channels == channels:
            self.skip_connection = nn.Identity()  # 使用 nn.Identity() 作为跳跃连接
            # nn.Identity() 是一个占位操作，它不改变传递给它的数据。
            # 这在输入和输出通道数相同时是有用的，因为不需要额外的操作来匹配通道数。
        elif use_conv:
            # 使用 conv_nd 创建一个空间卷积层作为跳跃连接。
            self.skip_connection = conv_nd(
                dims, channels, self.out_channels, 3, padding=1
            )
        else:
            # 使用一个1x1卷积（通过 conv_nd 创建）来改变通道数
...
class ResBlock(TimestepBlock):
    ...
    def __init__(
        ...
        # 处理时间步嵌入的层，使用线性变换，并根据 use_scale_shift_norm 决定输出的通道数。
    ...
    def _forward(self, x, emb):
        ...
        if is_video:
            num_frames = x.shape[2]
            x = rearrange(x, 'b c t h w -> (b t) c h w ')
        # 根据是否进行上/下采样，应用不同的处理流程
        if self.updown:
            in_rest, in_conv = self.in_layers[:-1], self.in_layers[-1]  # 对in_layers切片，分为0~n-1 和 n 两个部分
            # # h（处理后的特征）和 x（原始输入）
            h = in_rest(x)
            h = self.h_upd(h)
        ...
        # 应用输出层进行最终的数据处理
        if self.use_scale_shift_norm:  # 布尔值
            out_norm, out_rest = self.out_layers[0], self.out_layers[1:]  # 对out_layers切片，分为0 和 1~n 两个部分
            scale, shift = th.chunk(emb_out, 2, dim=1)  # 通过第二个维度dim维度分成两个部分
            h = out_norm(h) * (1 + scale) + shift
            # h 在经过规范化层后，会与 scale 相乘（缩放）并加上 shift（平移），从而实现条件调整。
...
class AttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        if num_head_channels == -1:
            ...
            self.num_heads = channels // num_head_channels  # 需要能整除才能赋值
        self.use_checkpoint = use_checkpoint
        self.norm = normalization(channels)  # 规范化
        self.qkv = conv_nd(1, channels, channels * 3, 1)  # 一个一维卷积层，用于生成查询（Q）、键（K）和值（V）
        # attention 注意力机制层。根据 use_new_attention_order 选择不同的实现。
        ...
        # 用于输出的投影层，通过 zero_module 初始化为接近零的权
    ...
    def _forward(self, x):
        b, c, *spatial = x.shape
        x = x.reshape(b, c, -1)  # 将输入数据 x 重塑为三维形状（batch, channel, -1）
        qkv = self.qkv(self.norm(x))  # 应用规范化层和一维卷积层 self.qkv 生成 Q、K、V
        h = self.attention(qkv)  # 应用注意力机制处理 Q、K、V。
        h = self.proj_out(h)  # 使用投影层 self.proj_out 对注意力机制的输出进行处理。
        return (x + h).reshape(b, c, *spatial)  # 将注意力机制的输出加回到原始输入 x 并恢复其原始空间维度
...
def count_flops_attn(model, _x, y):
    ...
    """
    b, c, *spatial = y[0].shape  # 提取注意力输出的批大小（b）、通道数（c）和空间维度（spatial）。
    num_spatial = int(np.prod(spatial))  # 计算所有空间维度的乘积，即空间位置的总数。
    # We perform two matmuls with the same number of ops.
    # The first computes the weight matrix, the second computes
    # the combination of the value vectors.
    matmul_ops = 2 * b * (num_spatial ** 2) * c
    # 计算这两个矩阵乘法操作所需的浮点运算数。
    # 每个乘法涉及 num_spatial ** 2 个元素的乘加操作，对于每个批和每个通道都需要执行。
